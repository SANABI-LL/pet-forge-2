...
**现象**：正脸、侧脸、抬头、低头的端点截图都好看，做成淡入淡出后，鼻梁、眼睛、腮红、耳朵和白脸边界在中途像两张贴图互相盖住。

**原因**：头脸方向变化是同一套 facial plane 的连续变形，不是图片切换。静态帧 crossfade 没有共享锚点、裁切和中间几何，所以斜向视线、指针跟随和 30 秒循环都会露馅。

**正确做法**：按 `routes/svg/conventions/head-motion-axis.md` 先建脸部语义锚点，再让 `yaw`、`pitch`、腮红裁切、腮影和耳朵共用同一套几何。端点确认后导出 `0% / 50% / 100%` 做防漂移对照。

### 16. 中线跳变通常是两套几何模型混用

**现象**：左右摇头或低头看向时，过正中位置鼻梁、白脸边缘或五官突然“啪”一下，像抖动，但不是普通数值 jitter。

**原因**：运动中的 `yaw=0` / `middle` 位姿切回 raw master path，而左右关键帧使用锚点模型或 mixer 模型。两套 path 的点顺序、鼻梁曲率或脸边轮廓不同，过中线时就会切模型。

**正确做法**：运动中的中性位姿必须和左右 / 上下关键帧来自同一几何模型。静态展示可以保留母版原脸，但 pointer-look、左右摇、斜向和低头循环里的 `0` 位姿要继续走同一套 face rig。
